...
使用之前需要在 [[https://console.cloud.google.com][Google Console]] 注册应用程序来得到 OAuth2 Client ID 和 Secret，备用。

在 =.folder= 文件里用 =xoath2= 验证类型：
...
之后在首次连接时 WanderLust 会依次提示输入 OAuth2 Client ID 和 Secret，输入
后会在浏览器打开选择 Google 账号的页面，同意授权后，会提示无法访问页面，但不要慌，
把地址栏里的 =code= 参数复制下来输入 WanderLust 就可以了。
